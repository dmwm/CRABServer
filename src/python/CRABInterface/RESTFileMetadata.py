...
from WMCore.REST.Validation import validate_ustr, validate_ustrlist, validate_num, validate_numlist
...
class RESTFileMetadata(RESTEntity):
    ...
    def validate(self, apiobj, method, api, param, safe):
        ...
        if method in ['PUT']:
            ...
            validate_ustr("taskname", param, safe, RX_TASKNAME, optional=False)
            validate_ustrlist("outfilelumis", param, safe, RX_LUMILIST)
            ...
            validate_ustrlist("inparentlfns", param, safe, RX_PARENTLFN)
            validate_ustr("globalTag", param, safe, RX_GLOBALTAG, optional=True)
            validate_ustr("jobid", param, safe, RX_JOBID, optional=True)
            ...
            validate_ustr("publishdataname", param, safe, RX_PUBLISH, optional=False)
            validate_ustr("appver", param, safe, RX_CMSSW, optional=False)
            validate_ustr("outtype", param, safe, RX_OUTTYPES, optional=False)
            validate_ustr("checksummd5", param, safe, RX_CHECKSUM, optional=False)
            validate_ustr("checksumcksum", param, safe, RX_CHECKSUM, optional=False)
            validate_ustr("checksumadler32", param, safe, RX_CHECKSUM, optional=False)
            validate_ustr("outlocation", param, safe, RX_CMSSITE, optional=False)
            validate_ustr("outtmplocation", param, safe, RX_CMSSITE, optional=False)
            validate_ustr("acquisitionera", param, safe, RX_TASKNAME, optional=False)#TODO Do we really need this?
            validate_ustr("outdatasetname", param, safe, RX_OUTDSLFN, optional=False)#TODO temporary, need to come up with a regex
            validate_ustr("outlfn", param, safe, RX_PARENTLFN, optional=False)
            validate_ustr("outtmplfn", param, safe, RX_PARENTLFN, optional=True)
            validate_num("events", param, safe, optional=False)
            validate_ustr("filestate", param, safe, RX_FILESTATE, optional=True)
            ...
            validate_ustr("taskname", param, safe, RX_TASKNAME, optional=False)
            validate_ustr("outlfn", param, safe, RX_LFN, optional=False)
            validate_ustr("filestate", param, safe, RX_FILESTATE, optional=False)
        elif method in ['GET']:
            validate_ustr("taskname", param, safe, RX_TASKNAME, optional=False)
            validate_ustr("filetype", param, safe, RX_OUTTYPES, optional=False)
            validate_num("howmany", param, safe, optional=True)
            validate_ustrlist("lfn", param, safe, RX_LFN)
        elif method in ['DELETE']:
            authz_operator()
            validate_ustr("taskname", param, safe, RX_TASKNAME, optional=True)
            validate_ustr("hours", param, safe, RX_HOURS, optional=True)
