...
class DataUserWorkflow(object):
    ...
    @retrieveUserCert
    def resubmit(self, workflow, siteblacklist, sitewhitelist, jobids, ASOinstance, objects, userdn, userproxy=None):
        """Request to Resubmit a workflow.

           :arg str workflow: a workflow name
           :arg int force: force to delete the workflows in any case; 0 no, everything else yes"""
        return self.workflow.resubmit(workflow, siteblacklist, sitewhitelist, jobids, ASOinstance, objects, userdn, userproxy)
    ...
    def kill(self, workflow, force, jobids, ASOinstance, objects, userdn, userproxy=None):
        """Request to Abort a workflow.
           ...
           :arg str userproxy: the user proxy retrieved by `retrieveUserCert`
           :arg int force: force to delete the workflows in any case; 0 no, everything else yes"""
        return self.workflow.kill(workflow, force, jobids, ASOinstance, objects, userdn, userproxy)
