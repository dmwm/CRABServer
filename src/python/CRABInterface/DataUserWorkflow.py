...
:returns: a dict which contaians details of the request"""
    ...
    @retrieveUserCert
    def resubmit(self, workflow, siteblacklist, sitewhitelist, jobids, ASOinstance, objects, userdn, hostkey, hostcert, userproxy=None):
        """Request to Resubmit a workflow.

:arg str workflow: a workflow name
:arg int force: force to delete the workflows in any case; 0 no, everything else yes"""
        return self.workflow.resubmit(workflow, siteblacklist, sitewhitelist, jobids, ASOinstance, objects, userdn, hostkey, hostcert, userproxy)
...
:arg str userproxy: the user proxy retrieved by `retrieveUserCert`
:arg int force: force to delete the workflows in any case; 0 no, everything else yes"""
        return self.workflow.kill(workflow, force, jobids, ASOinstance, objects, userdn, hostkey, hostcert, userproxy)
