...
class TaskHandler:
    ...
    def __init__(
        self, task, procnum, crabserver, config, workFunction, createTempDir=False
    ):
        """Initializer
    ...
    def createTempDir(self):
        ...
        if hasattr(self.config, "TaskWorker") and hasattr(
            ...
            tempDir = tempfile.mkdtemp(
                prefix="_" + self.taskname, dir=self.config.TaskWorker.scratchDir
            )
        else:
    ...
    def executeAction(self, nextinput, work):
        ...
        try:
            ...
            uploadWarning(
                self.crabserver, self.task["tm_taskname"], str(e), self.logger
            )
            updateTaskStatus(
                self.crabserver,
                taskName=self.task["tm_taskname"],
                status="SUBMITREFUSED",
                logger=self.logger,
            )
            ...
            ) from twe  # TaskWorker error, do not add traceback
            ...
            raise WorkerHandlerException(
                msg
            ) from exc  # Errors not foreseen. Print everything!
        finally:
            logpath = (
                self.config.TaskWorker.logsDir
                + f"/tasks/{self.task['tm_username']}/{self.taskname}.log"
            )
            if (
                os.path.isfile(logpath) and "user_proxy" in self.task
            ):  # the user proxy might not be there if myproxy retrieval failed
                try:
                    uploadToS3(
    ...
    def actionWork(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        for action in self.getWorks():
            ...
            self.logger.info(
                "Finished %s on %s in %d seconds", str(action), self.taskname, t1 - t0
            )
...
def handleNewTask(resthost, dbInstance, config, task, procnum, *args, **kwargs):
    ...
    handler = TaskHandler(
        task, procnum, crabserver, config, "handleNewTask", createTempDir=True
    )
    ...
    with config.TaskWorker.envForCMSWEB:
        resourceCatalog = CRICService(
            logger=handler.logger, configDict={"cacheduration": 1, "pycurl": True}
        )
    ...
    handler.addWork(
        StageoutCheck(
            ...
            crabserver=crabserver,
            procnum=procnum,
            rucioClient=privilegedRucioClient,
        )
    )
    if task["tm_job_type"] == "Analysis":
        if task.get("tm_input_dataset"):
            ...
            raise SubmissionRefusedException(
                "Neither inputDataset nor userInputFiles specified", retry=0
            )
...
def handleResubmit(resthost, dbInstance, config, task, procnum, *args, **kwargs):
    ...
    handler.addWork(
        DagmanResubmitter(config=config, crabserver=crabserver, procnum=procnum)
    )
...
def handleKill(resthost, dbInstance, config, task, procnum, *args, **kwargs):
    ...
    handler.addWork(
        MyProxyLogon(
            config=config, crabserver=crabserver, procnum=procnum, myproxylen=60 * 5
        )
    )
