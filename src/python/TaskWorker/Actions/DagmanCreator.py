...
def makeLFNPrefixes(task):
    ...
    publish_info = task["tm_publish_name"].rsplit(
        "-", 1
    )  # publish_info[0] is the publishname or the taskname
    ...
    if splitlfn[2] == "user":
        ...
        temp_dest = os.path.join(
            ...
            "user",
            tmp_user,
            *(splitlfn[4:] + [primaryds, publish_info[0], timestamp]),
        )
    else:
        temp_dest = os.path.join(
            "/",
            ...
            "user",
            tmp_user,
            *(splitlfn[3:] + [primaryds, publish_info[0], timestamp]),
        )
...
def validateLFNs(path, outputFiles):
    ...
    dirCounter = (
        "0001"  # need to be same length as 'counter' used later in makeDagSpecs
    )
    ...
    for origFile in outputFiles:
        ...
        if (
            len(nameParts) == 2
        ):  # filename ends with .<something>, put jobId before the dot
...
class DagmanCreator(TaskAction):
    ...
    def __init__(
        self, config, crabserver, resourceCatalog=None, procnum=-1, rucioClient=None
    ):
    ...
    def makeJobSubmit(self, task):
        ...
        def pythonListToClassAdValue(aList):
            ...
            quotedItems = quotedItems.lstrip("[").rstrip(
                "]"
            )  # remove square brackets [ ]
            value = (
                "{" + quotedItems + "}"
            )  # make final string adding the curly brackets { }
            return value

        jobSubmit["My.CRAB_SiteBlacklist"] = pythonListToClassAdValue(
            task["tm_site_blacklist"]
        )
        jobSubmit["My.CRAB_SiteWhitelist"] = pythonListToClassAdValue(
            task["tm_site_whitelist"]
        )
        jobSubmit["My.CRAB_AdditionalOutputFiles"] = pythonListToClassAdValue(
            task["tm_outfiles"]
        )
        jobSubmit["My.CRAB_EDMOutputFiles"] = pythonListToClassAdValue(
            task["tm_edm_outfiles"]
        )
        jobSubmit["My.CRAB_TFileOutputFiles"] = pythonListToClassAdValue(
            task["tm_outfiles"]
        )
        ...
        jobSubmit["My.CRAB_UserRole"] = (
            classad.quote(userRole) if userRole else "undefined"
        )
        userGroup = task["tm_user_group"]
        jobSubmit["My.CRAB_UserGroup"] = (
            classad.quote(userGroup) if userGroup else "undefined"
        )
        jobSubmit["My.CRAB_TaskWorker"] = classad.quote(
            getattr(self.config.TaskWorker, "name", "unknown")
        )
        retry_aso = (
            "1" if getattr(self.config.TaskWorker, "retryOnASOFailures", True) else "0"
        )
        jobSubmit["My.CRAB_RetryOnASOFailures"] = retry_aso
        jobSubmit["My.CRAB_ASOTimeout"] = str(
            getattr(self.config.TaskWorker, "ASORucioTimeout", 0)
        )
        ...
        jobSubmit["My.CRAB_Id"] = classad.quote(
            "$(count)"
        )  # count macro will be defined in VARS line in the DAG file
        ...
        jobSubmit["My.CRAB_SubmitterIpAddr"] = classad.quote(
            task["tm_submitter_ip_addr"]
        )
        jobSubmit["My.CRAB_TaskLifetimeDays"] = str(TASKLIFETIME // 24 // 60 // 60)
        jobSubmit["My.CRAB_TaskEndTime"] = str(
            int(task["tm_start_time"]) + TASKLIFETIME
        )
        jobSubmit["My.CRAB_SplitAlgo"] = classad.quote(task["tm_split_algo"])
        jobSubmit["My.CRAB_AlgoArgs"] = classad.quote(
            str(task["tm_split_args"])
        )  # from dict to str before quoting
        ...
        transferOutputs = (
            "1" if task["tm_transfer_outputs"] == "T" else "0"
        )  # Note: this must always be 0 for probe jobs, is taken care of in PreJob.py
        ...
        jobSubmit["My.REQUIRED_MINIMUM_MICROARCH"] = str(
            matchInfo["required_minimum_microarch"]
        )
        ...
        groups = ",".join(
            groups
        )  # from the set {'g1','g2'...,'gN'} to the string 'g1,g2,..gN'
        ...
        jobSubmit["My.JOBGLIDEIN_CMSSite"] = classad.quote(
            '$$([ifThenElse(GLIDEIN_CMSSite is undefined, "Unknown", GLIDEIN_CMSSite)])'
        )
        ...
        jobSubmit["job_ad_information_attrs"] = (
            "MATCH_EXP_JOBGLIDEIN_CMSSite, JOBGLIDEIN_CMSSite, RemoteSysCpu, RemoteUserCpu"
        )
        ...
        jobSubmit["LeaveJobInQueue"] = (
            "ifThenElse((JobStatus=?=4 || JobStatus=?=3) "
        ...
        jobSubmit["transfer_output_files"] = (
            "jobReport.json.$(count), WMArchiveReport.json.$(count)"
        )
        ...
        if os.path.exists("CMSRunAnalysis.tar.gz"):
            ...
            raise TaskWorkerException(
                f"Cannot find CMSRunAnalysis.tar.gz inside the cwd: {os.getcwd()}"
            )
        if os.path.exists("TaskManagerRun.tar.gz"):
            additional_environment_options += " CRAB_TASKMANAGER_TARBALL=local"
        else:
            raise TaskWorkerException(
                f"Cannot find TaskManagerRun.tar.gz inside the cwd: {os.getcwd()}"
            )
        additional_input_file += (
            ", sandbox.tar.gz"  # it will be present on SPOOL_DIR after dab_bootstrap
        )
        ...
        jobSubmit["transfer_input_files"] = (
            f"CMSRunAnalysis.sh, cmscp.py{additional_input_file}"
        )
        ...
        jobSubmit["Environment"] = classad.quote(
            f"SCRAM_ARCH=$(CRAB_JobArch){additional_environment_options}"
        )
        jobSubmit["should_transfer_files"] = "YES"
        jobSubmit["use_x509userproxy"] = "true"

        arch = task["tm_job_arch"].split("_")[
            0
        ]  # extracts "slc7" from "slc7_amd64_gcc10"
        ...
        periodicRemove = (
            "( (JobStatus =?= 5) && (time() - EnteredCurrentStatus > 7*60) )"  # a)
        )
        periodicRemove += "|| ( (JobStatus =?= 1) && (time() - EnteredCurrentStatus > 7*24*60*60) )"  # b)
        periodicRemove += "|| ( (JobStatus =?= 2) && ( "  # c)
        periodicRemove += (
            "(MemoryUsage =!= UNDEFINED && MemoryUsage > RequestMemory)"  # c) 1)
        )
        periodicRemove += (
            "|| (MaxWallTimeMinsRun * 60 < time() - EnteredCurrentStatus)"  # c) 2)
        )
        ...
        periodicRemoveReason += (
            '"Removed due to idle time limit",'  # set this reasons. Else
        )
        ...
        periodicRemoveReason += (
            f'ifThenElse(DiskUsage > {MAX_DISK_SPACE}, "Removed due to disk usage",'
        )
        ...
        for extraJdl in literal_eval(task["tm_extrajdl"]):
            ...
            if name.startswith("+"):
                name = name.replace(
                    "+", "My.", 1
                )  # make sure JDL line starts with My. instead of +
        ...
        if task["tm_user_config"]["requireaccelerator"]:
            ...
            if task["tm_user_config"]["acceleratorparams"]:
                gpuMemoryMB = task["tm_user_config"]["acceleratorparams"].get(
                    "GPUMemoryMB", None
                )
                cudaCapabilities = task["tm_user_config"]["acceleratorparams"].get(
                    "CUDACapabilities", None
                )
                cudaRuntime = task["tm_user_config"]["acceleratorparams"].get(
                    "CUDARuntime", None
                )
                if gpuMemoryMB:
    ...
    def getPreScriptDefer(self, task, jobid):
        ...
        extrajdls = literal_eval(
            task["tm_extrajdl"]
        )  # from "['a','b',...]" to a python list
        for ej in extrajdls:
            if ej.startswith("+"):
                ej = ej.replace("+", "My.", 1)
    ...
    def makeDagSpecs(
        ...
        for job in jobgroup.getJobs():
            if task["tm_use_parent"] == 1:
                ...
                inputFiles = json.dumps(
                    [inputfile["lfn"] for inputfile in job["input_files"]]
                )
            ...
            counter = (
                f"{(i // 1000):04d}"  # counter=0000 for i<999, 1 for 1000<i<1999 etc.
            )
            ...
            if lastDirectDest != directDest:
                ...
                lastDirectPfn = getWritePFN(
                    self.rucioClient,
                    siteName=task["tm_asyncdest"],
                    lfn=directDest,
                    operations=["write"],
                    logger=self.logger,
                )
            ...
            nodeSpec = {
                ...
                "lheInputFiles": "tm_generator" in task
                and task["tm_generator"] == "lhe",
                "eventsPerLumi": task["tm_events_per_lumi"],
                "maxRuntime": task["max_runtime"]
                * 60,  # the job script takes seconds, internal units are minutes
                "sw": task["tm_job_sw"],
                "block": block,
                "destination": pfns,
    ...
    def prepareJobArguments(self, dagSpecs, task):
        ...
        for dagspec in dagSpecs:
            argDict = {}
            argDict["inputFileList"] = (
                f"job_input_file_list_{dagspec['count']}.txt"  #'job_input_file_list_1.txt'
            )
            ...
            argDict["userSandbox"] = task[
                "tm_user_sandbox"
            ]  # SB we could simply hardocode 'sandbox.tar.gz'
    ...
    def createSubdag(self, splitterResult, **kwargs):
        ...
        dagSpecs = (
            []
        )  # there will be a dagSpec for every node to be written in the DAG file
        subdags = (
            []
        )  # in automatic splitting a DAG may contain a list of one or more subdags to start

        # disable direct stageout for rucio tasks
        if kwargs["task"]["tm_output_lfn"].startswith("/store/user/rucio") or kwargs[
            "task"
        ]["tm_output_lfn"].startswith("/store/group/rucio"):
        ...
        kwargs["task"]["numautomjobretries"] = getattr(
            self.config.TaskWorker, "numAutomJobRetries", 2
        )
        ...
        tailruntime = int(
            max(
                ...
                getattr(self.config.TaskWorker, "automaticTailRuntimeFraction", 0.2)
                * runtime,
            )
        )

        overhead = getattr(
            self.config.TaskWorker, "automaticProcessingOverheadMins", 60
        )
        ...
        if kwargs["task"]["tm_split_algo"] == "Automatic":
            if stage == "conventional":
                ...
                # given to CMSSW
                kwargs["task"]["tm_maxjobruntime"] = min(
                    runtime + overhead, kwargs["task"]["tm_maxjobruntime"]
                )
            elif stage == "tail":
                kwargs["task"]["max_runtime"] = -1

        outfiles = (
            kwargs["task"]["tm_outfiles"]
            + kwargs["task"]["tm_tfile_outfiles"]
            + kwargs["task"]["tm_edm_outfiles"]
        )
        ...
        global_blacklist = set(
            self.loadJSONFromFileInScratchDir("blacklistedSites.txt")
        )
        ...
        acceleratorsites = set(
            self.loadJSONFromFileInScratchDir("acceleratorSites.json")
        )
        self.logger.debug(
            "Accelerator site from pilot pool: %s", list(acceleratorsites)
        )
        ...
        if self.isGlobalBlacklistIgnored(kwargs) or (
            hasattr(self.config.TaskWorker, "ActivitiesToRunEverywhere")
            and kwargs["task"]["tm_activity"]
            in self.config.TaskWorker.ActivitiesToRunEverywhere
        ):
            global_blacklist = set()
        ...
        bannedOutDestinations = self.crabserver.get(
            api="info", data={"subresource": "bannedoutdest"}
        )[0]["result"][0]
        ...
        if siteWhitelist & global_blacklist:
            ...
            self.uploadWarning(
                msg, kwargs["task"]["user_proxy"], kwargs["task"]["tm_taskname"]
            )
        ...
        for jobgroup in splitterResult[0]:
            ...
            if kwargs["task"]["tm_user_config"]["requireaccelerator"]:
                ...
                if availablesites:
                    ...
                    blocksWithBannedLocations = blocksWithBannedLocations.union(
                        jgblocks
                    )
                    continue
            ...
            if siteWhitelist or siteBlacklist:
                ...
                msg += (
                    f" This is expected to result in DESIRED_SITES = {list(available)}"
                )
        ...
        restHostForSchedd = kwargs["task"][
            "resthost"
        ]  # SB better to use self.crabserver.server['host']
        dag = DAG_HEADER.format(
            nodestate=(
                f".{parentDag}"
                if parentDag
                else (".0" if stage == "processing" else "")
            ),
            resthost=restHostForSchedd,
        )
        if stage == "probe":
            dagSpecs = dagSpecs[
                : getattr(self.config.TaskWorker, "numAutomaticProbes", 5)
            ]
        for dagSpec in dagSpecs:
        ...
        if stage in ("probe", "processing"):
            ...
            if stage == "processing" and len(dagSpecs) > getattr(
                self.config.TaskWorker, "minAutomaticTailSize", 100
            ):
                subdagCompletions = getattr(
                    self.config.TaskWorker, "minAutomaticTailTriggers", [50, 80, 100]
                )

            for n, percent in enumerate(
                subdagCompletions, 0 if stage == "probe" else 1
            ):
                subdagSpec = {
                    "count": n,
                    "stage": nextStage,
        ...
        with getLock("splitting_data"):
            ...
            try:
                ...
                try:
                    ...
                    self.logger.debug(
                        "First iteration: creating run and lumi from scratch"
                    )
                try:
                    ...
                    self.logger.debug(
                        "First iteration: creating inputfiles from scratch"
                    )
                tfd = tarfile.open("run_and_lumis.tar.gz", "w:gz")
                tfd2 = tarfile.open("input_files.tar.gz", "w:gz")
                for dagSpec in dagSpecs:
                    job_lumis_file = os.path.join(
                        tempDir, "job_lumis_" + str(dagSpec["count"]) + ".json"
                    )
                    ...
                    job_input_file_list = os.path.join(
                        tempDir2,
                        "job_input_file_list_" + str(dagSpec["count"]) + ".txt",
                    )
        ...
        if maxidle == -1:
            ...
            maxidle = int(max(MAX_IDLE_JOBS, kwargs["task"]["jobcount"] * 0.1))
        ...
        if maxpost == -1:
            ...
            maxpost = int(max(MAX_POST_JOBS, kwargs["task"]["jobcount"] * 0.1))
    ...
    def getHighPrioUsers(self, egroups):
        ...
        try:
            ...
            msg = (
                "Error when getting the high priority users list."
    ...
    def executeInternal(self, *args, **kw):
        ...
        if self.runningInTW:
            ...
            try:
                checkS3Object(
                    ...
                    taskname=taskname,
                    username=username,
                    tarballname=sandboxName,
                    logger=self.logger,
                )
                kw["task"][
                    "tm_user_sandbox"
                ] = sandboxTarBall  # SB no need for this either ! it is only there to reuse same name in prepareArguments. A variable in self would be much cleaner, of even one constant to ensure same name in AdjustSites and CRABClient
            except Exception as ex:
                raise TaskWorkerException(
                    "The CRAB server backend could not find the input sandbox with your code "
        ...
        kw["task"]["resthost"] = self.crabserver.server[
            "host"
        ]  # SB most likely not needed, see Line 1027
        kw["task"][
            "dbinstance"
        ] = (
            self.crabserver.getDbInstance()
        )  # SB never used in here nor in DagmanSubmitter ??
        params = {}
        ...
        jobGroups = splitterResult[
            0
        ]  # the first returned value of Splitter action is the splitterFactory output
    ...
    def execute(self, *args, **kw):
        ...
        try:
            os.chdir(kw["tempDir"])
            jobSubmit, params, inputFiles, splitterResult = self.executeInternal(
                *args, **kw
            )
            return TaskWorker.DataObjects.Result.Result(
                task=kw["task"], result=(jobSubmit, params, inputFiles, splitterResult)
            )
        finally:
            os.chdir(cwd)
        ...
        for site in sites:
            if "*" in site:
                ...
                expanded = [
                    str(s)
                    for s in (
                        self.resourceCatalog.getAllPhEDExNodeNames()
                        if pnn
                        else self.resourceCatalog.getAllPSNs()
                    )
                    if sitere.match(s)
                ]
                self.logger.debug(
                    "Site %s expanded to %s during validate", site, expanded
                )
                if not expanded:
                    raise ConfigException(
                        "Remote output data site not valid, Cannot expand site %s to anything"
                        % site
                    )
    ...
    def _checkASODestination(self, site, bannedOutDestinations=[]):
        ...
        if site in expandedBannedDestinations:
            raise ConfigException(
                "Remote output data site is banned, The output site you specified in the Site.storageSite parameter (%s) is blacklisted (banned sites: %s)"
                % (site, bannedOutDestinations)
            )
    ...
    def _checkSite(self, site, pnn=False):
        ...
        if site not in sites:
            raise ConfigException(
                "A site name %s that user specified is not in the list of known CMS %s"
                % (site, "Storage nodes" if pnn else "Processing Site Names")
            )
...
if __name__ == "__main__":
    ...
    envForCMSWEB = newX509env(
        X509_USER_CERT=config.TaskWorker.cmscert, X509_USER_KEY=config.TaskWorker.cmskey
    )
    ...
    with envForCMSWEB:
        ...
        creator = DagmanCreator(
            config, crabserver=None, resourceCatalog=resourceCatalog
        )

        assert (
            creator._checkSite("T2_US_Florida", pnn=False) is None
        ), "Site T2_US_Florida is valid"
        try:
            creator._checkSite("T2_TH_Bangkok", pnn=False)
        except ConfigException as ex:
            assert (
                str(ex)
                == "A site name T2_TH_Bangkok that user specified is not in the list of known CMS Processing Site Names"
            ), "Site T2_TH_Bangkok are not existing and should be invalid"
        assert (
            creator._checkASODestination("T2_US_Florida", MOCK_BANNED_OUT_DESTINATIONS)
            is None
        ), "Site: T2_US_Florida, Should not be banned"
        try:
            creator._checkASODestination(
                "T2_UK_SGrid_Bristol", MOCK_BANNED_OUT_DESTINATIONS
            )
        except ConfigException as ex:
            assert (
                str(ex)
                == "Remote output data site is banned, The output site you specified in the Site.storageSite parameter (T2_UK_SGrid_Bristol) is blacklisted (banned sites: ['T2_UK_SGrid_Bristol', 'T2_US_MIT'])"
            ), "ASO destination is banned"
        assert sorted(creator._expandSites(["T2_US*", "T2_UK*"])) == sorted(
            [
                "T2_US_Caltech",
                "T2_US_Florida",
                "T2_US_MIT",
