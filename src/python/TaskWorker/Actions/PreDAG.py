...
class PreDAG():
    ...
    def executeInternal(self, *args):
        ...
        try:
            ...
            with config.TaskWorker.envForCMSWEB:
                resourceCatalog = CachedCRICService(logger=self.logger, configDict={"cacheduration": 1, "pycurl": True})
                creator = DagmanCreator(config, crabserver=None, resourceCatalog=resourceCatalog, rucioClient=rucioClient)
