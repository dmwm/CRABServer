...
class DBSDataDiscovery(DataDiscovery):
    ...
    def __init__(
        self, *args, rucioClient=None, **kwargs
    ...
    def checkDatasetStatus(self, dataset, kwargs):
        """as the name says"""
        res = self.dbs.dbs.listDatasets(
            dataset=dataset, detail=1, dataset_access_type="*"
        )
        if not res:
            raise SubmissionRefusedException(
                f"Cannot find dataset {dataset} in {self.dbsInstance} DBS instance"
            )
        if len(res) > 1:
            raise SubmissionRefusedException(
                f"Found more than one dataset while checking in DBS the status of {dataset}"
            )
        res = res[0]
        # import pprint
    ...
    def validateInputUserFiles(self, dataset=None, files=None):
        ...
        try:
            ...
            raise SubmissionRefusedException(
                f"input file is not a valid LFN: {file}"
            ) from ex
        filesInDataset = self.dbs.listDatasetFiles(datasetPath=dataset)
        for file in files:
            if not file in filesInDataset:
                raise SubmissionRefusedException(
                    f"input file use does not belong to input dataset: {file}"
                )
    ...
    def validateInputBlocks(dataset=None, blocks=None):
        ...
        try:
            ...
            raise SubmissionRefusedException(
                f"input block is not a valid block name: {block}"
            ) from ex
        for block in blocks:
            if not block.split("#")[0] == dataset:
                raise SubmissionRefusedException(
                    f"input block does not belong to input dataset: {block}"
                )
    ...
    def executeInternal(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        if kwargs["task"]["tm_dbs_url"]:
            ...
            dbsurl = (
                "https://cmsweb.cern.ch/dbs/prod/global/DBSReader"  # a sensible default
            )
        ...
        if useRucioForLocations:
            ...
            try:
                for blockName in list(blocks):
                    partialReplicas = set()
                    fullReplicas = set()
                    response = self.rucioClient.list_dataset_replicas(
                        scope=scope, name=blockName, deep=True
                    )
                    sizeBytes = 0
        ...
        if not locationsFoundWithRucio:
            ...
            if isUserDataset:
                self.logger.info(
                    "USER dataset. Looking up data locations using origin site in DBS"
                )
                try:
        ...
        if secondaryDataset:
            if secondaryDataset.endswith("USER"):
                self.logger.info(
                    "Secondary dataset is USER. Looking up data locations using origin site in DBS"
                )
                try:
                    secondaryLocationsMap = self.dbs.listFileBlockLocation(
                        list(secondaryBlocks)
                    )
                except Exception as ex:
                    raise TaskWorkerException(
                ...
                self.logger.info(
                    "Trying data location of secondary dataset blocks with Rucio"
                )
                try:
        ...
        if notAllOnDisk and usePartialDataset:
            ...
            for block in blocksWithLocation:  # all blocks known to Rucio
                if (
                    not block in locationsMap
                ):  # no available disk replica for this block
                    if block in partialLocationsMap:
                        locationsMap[block] = partialLocationsMap[
                            block
                        ]  # use partial replicas

        # check for tape recall
        # if set(locationsMap.keys()) != set(blocksWithLocation):
        if requestTapeRecall:
            msg = self.executeTapeRecallPolicy(
                inputDataset, inputBlocks, totalSizeBytes
            )
            dataToRecall = inputDataset if not inputBlocks else list(blocksWithLocation)
            self.requestTapeRecall(
                dataToRecall=dataToRecall,
                sizeToRecall=totalSizeBytes,
                tapeLocations=tapeLocations,
                system="Rucio",
                msgHead=msg,
            )
        ...
        try:
            filedetails = self.dbs.listDatasetFileDetails(
                inputDataset, getParents=True, getLumis=needLumiInfo, validFileOnly=0
            )
            if inputBlocks:
    ...
    def executeTapeRecallPolicy(self, inputDataset, inputBlocks, totalSizeBytes):
        ...
        maxTierToBlockRecallSizeTB = getattr(
            self.config.TaskWorker, "maxTierToBlockRecallSizeTB", 0
        )
        maxAnyTierRecallSizeTB = getattr(
            self.config.TaskWorker, "maxAnyTierRecallSizeTB", 0
        )
        maxRecallPerUserTB = getattr(
            self.config.TaskWorker, "maxRecallPerUserTB", 100e3
        )  # defaul to 100PB for testing

        # how much this user is recalling already
        self.logger.debug("Find how much user %s is recalling already", self.username)
        usage = getRucioUsage(
            self.rucioClient, self.username, activity="Analysis TapeRecall"
        )  # in Bytes
        ...
        if not userHasQuota:
            ...
            overQmsg += (
                f"{usageTB + totalSizeTB}TB, max allowed: {maxRecallPerUserTB}TB"
            )
        if (
            dataTier in getattr(self.config.TaskWorker, "tiersToRecall", [])
            or totalSizeTB < maxAnyTierRecallSizeTB
        ):
...
if __name__ == "__main__":
    ...
    config.TaskWorker.tiersToRecall = [
        "AOD",
        "AODSIM",
        "MINIAOD",
        "MINIAODSIM",
    ...
    discovery = DBSDataDiscovery(
        config=config, resourceCatalog=resourceCatalog, rucioClient=rucioClient
    )
    userConfig = {"partialdataset": False, "inputblocks": blockList}
    discoveryOutput = discovery.execute(
        task={
            "tm_nonvalid_input_dataset": "T",
            "tm_use_parent": 0,
            "user_proxy": "None",
            "tm_input_dataset": dbsDataset,
