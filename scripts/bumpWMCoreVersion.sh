#! /usr/bin/env bash
# How to use script:
#  - bumping to latest version > ./scripts/bumpWMCoreVersion.sh
#  - bumping to specific version > ./scripts/bumpWMCoreVersion.sh 2.3.7.6
...
if [ -z "$1" ]
  then
    ...
    echo -e "The bumping WMCore version to $WMCORE_RELEASE_TAG\n"
fi
...
if [[ "$PWD" =~ CRABServer ]]; then
  ...
  echo -e "Please run this script in the CRABServer code base directory."
fi
