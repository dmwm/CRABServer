...
class submit_thread(threading.Thread):
    ...
    def run(self):
        ...
        fileDoc['list_of_fts_instance'] = ['https://fts3-cms.cern.ch:8446/' for _ in self.files]
...
def perform_transfers(inputFile, lastLine, _lastFile, ftsContext, rucioClient):
    ...
    with open(inputFile) as _list:
        ...
        if len(transfers) > 0:
            ...
            for jobid in jobids:
                logging.info("Monitor link: https://fts3-cms.cern.ch:8449/fts3/ftsmon/#/job/%s", jobid)
