...
def defineTheBuild(self, dist, systemname, patchx=''):
    # Expand various sources.
    docroot = "doc/build/html"
    system = systems[systemname]
    #binsrc = sum((glob("bin/%s" % x) for x in system['bin']), [])

    #pyversion = sys.version.split()[0] UNUSED
    #pylibdir = '%slib/python%s/site-packages' % (patchx, pyversion[0:3]) UNUSED
    ...
    if os.path.exists(docroot):
        for dirpath, _dirs, files in os.walk(docroot):
            dist.data_files.append(("%sdoc%s" % (patchx, dirpath[len(docroot):]),
...
class BuildCommand(Command):
    ...
    def finalize_options(self):
        ...
        defineTheBuild(self, self.distribution, self.system, '')

        # Force rebuild.
        shutil.rmtree("%s/build" % getRelativePath(), True)
...
class InstallCommand(install):
    ...
    def finalize_options(self):
        ...
        defineTheBuild(self, self.distribution, self.system, (self.patch and 'x') or '')
    ...
    def run(self):
        for cmdname in self.get_sub_commands():
            cmd = self.distribution.get_command_obj(cmdname)
            cmd.distribution = self.distribution
            if cmdname == 'install_data':
                dataDir = '/xdata' if self.patch else '/data'
                cmd.install_dir = self.prefix + dataDir
            else:
                cmd.install_dir = self.install_lib
            cmd.ensure_finalized()
            self.run_command(cmdname)
            self.distribution.have_run[cmdname] = 1
...
def getWebDir():
    res = []
    for dirname in ['css', 'html', 'script']:
        for root, _dirs, files in os.walk('src/'+dirname):
